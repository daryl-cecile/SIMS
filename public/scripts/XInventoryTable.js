...
class XInventoryItem extends HTMLElement {
    ...
    updatePreviews() {
        if (this.values === null)
            return;
        this.shadow.querySelector('input[type=checkbox]').checked = this.isChecked;
    ...
    updateVisuals() {
        if (this.model == null)
            return;
        if (this.values === null)
            this.values = Object.assign({}, this.model);
        let eb = Tools.ElementBuilder;
        this.shadow.innerHTML = "";
        this.shadow.appendChild(eb("link", {
            href: "/public/styles/xinventory.css",
                ...
                d.appendChild(eb("label").create(lbl => {
                    ...
                    lbl.appendChild(eb("textarea", { name: "notices" }, this.model.item.notices.map(n => n.title).join("\n")).create(i => {
                        i.addEventListener('input', function () {
                            that.values.item.notices = this.value.split("\n").map(n => {
                                return { title: n };
                            });
                        });
                    }));
...
//# sourceMappingURL=XInventoryTable.js.map