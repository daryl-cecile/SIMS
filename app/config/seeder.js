...
(async function () {
    ...
    await seedStock(await seedItem("Sample Item", "A sample item", 5, new Date(), [
        await seedNotice("Store in a wet place"),
        await seedNotice("Keep away from adults")
    ]), 12, await seedLocation("main room", "somewhere"));
    await seedStock(await seedItem("Sample Item TWO", "A second sample item", 12, new Date(), [
        await seedNotice("Keep away from adults")
    ]), 10, await seedLocation("little room", "somewhere"));
})();
