...
(function (Passport) {
    ...
    async function MakeAdminAccessOnly(title, res) {
        let authCheck = await this.isAuthenticated();
        if (authCheck.object.isSuccessful) {
            let acc = authCheck.object.payload['user'];
            if (await UserService_1.UserService.hasPermission(acc, "MANAGE") === false) {
    ...
    Passport.MakeAdminAccessOnly = MakeAdminAccessOnly;
